...
### 3d鸟群
3d的实现和2d其实原理类似，唯一要注意的地方就是对象的转向问题，会使用四元数来进行处理，关键就是` this.rot.setFromQuaternion(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), new THREE.Vector3(this.ve.x, this.ve.y, this.ve.z).normalize()))`
